...
function navigateTo(viewName) {
    ...
    if (viewName === 'model-calib') {
        if (typeof volcubeBuilder !== 'undefined') {
            volcubeBuilder.init();
        }
    }
    if (viewName === 'graph') {
...
async function loadStressComparison(scenarioIds = ['base']) {
    try {
        ...
        const loadPromises = selectedIds.map(scenarioId =>
            fetchJson(`${API_BASE}/stress/${scenarioId}`, {}, 'Stress scenario load failed')
        );
...
const scenarioAnalysis = (function() {
    ...
    async function runScenarioWithAPI() {
        ...
        try {
            ...
            if (!curveId) {
                throw new Error('No curve available. Build a curve first using Curve Builder.');
            }
