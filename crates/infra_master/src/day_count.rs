...
//!
//! ```
//! use infra_master::{Date, DayCountConvention};
//!
//! let start = Date::from_ymd(2024, 1, 1).unwrap();
//! let end = Date::from_ymd(2024, 7, 1).unwrap();
//!
//! let yf = DayCountConvention::Actual365Fixed.year_fraction_dates(start, end);
...
impl DayCountConvention {
    ...
    ///
    /// ```
    /// use infra_master::DayCountConvention;
    ///
    /// assert_eq!(DayCountConvention::Actual365Fixed.name(), "ACT/365");
    /// assert_eq!(DayCountConvention::Actual360.name(), "ACT/360");
    /// assert_eq!(DayCountConvention::Thirty360Bond.name(), "30/360");
    /// ```
    #[must_use]
    pub fn name(&self) -> &'static str {
        match self {
            DayCountConvention::Actual360 => "ACT/360",
            DayCountConvention::Actual365Fixed => "ACT/365",
            DayCountConvention::Actual36525 => "ACT/365.25",
            DayCountConvention::ActualActualIsda => "ACT/ACT ISDA",
            DayCountConvention::Thirty360Bond => "30/360",
            DayCountConvention::Thirty360European => "30E/360",
            DayCountConvention::ThirtyE360Isda => "30E/360 ISDA",
        }
    }

    /// Calculate the year fraction between two NaiveDate values.
    ///
    /// # Panics
    /// Panics if `start > end`.
    ///
    ...
    /// Calculate year fraction using Date type.
    ///
    ...
    /// let yf_neg = DayCountConvention::Actual365Fixed.year_fraction_dates(end, start);
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s.to_uppercase().replace(['/', ' ', '.'], "").as_str() {
            ...
            "ACT365" | "ACTUAL365" | "A365" | "ACT365FIXED" | "ACTUAL365FIXED" => {
                Ok(DayCountConvention::Actual365Fixed)
            }
            "ACT36525" | "ACTUAL36525" => Ok(DayCountConvention::Actual36525),
            "ACTACTISDA" | "ACTUALACTUALISDA" | "ACTACT" => {
                Ok(DayCountConvention::ActualActualIsda)
            }
            "30360" | "THIRTY360" | "30360BOND" => Ok(DayCountConvention::Thirty360Bond),
            "30E360" | "30360EUROPEAN" => Ok(DayCountConvention::Thirty360European),
            "30E360ISDA" => Ok(DayCountConvention::ThirtyE360Isda),
...
mod tests {
    ...
    fn test_name() {
        assert_eq!(DayCountConvention::Actual360.name(), "ACT/360");
        assert_eq!(DayCountConvention::Actual365Fixed.name(), "ACT/365");
        assert_eq!(DayCountConvention::Actual36525.name(), "ACT/365.25");
        assert_eq!(DayCountConvention::ActualActualIsda.name(), "ACT/ACT ISDA");
        assert_eq!(DayCountConvention::Thirty360Bond.name(), "30/360");
        assert_eq!(DayCountConvention::Thirty360European.name(), "30E/360");
        assert_eq!(DayCountConvention::ThirtyE360Isda.name(), "30E/360 ISDA");
    }

    #[test]
    fn test_display() {
        assert_eq!(format!("{}", DayCountConvention::Actual365Fixed), "ACT/365");
        assert_eq!(format!("{}", DayCountConvention::Thirty360Bond), "30/360");
    }
    ...
    fn test_same_date_returns_zero() {
        ...
        for dcc in [
            ...
            assert_eq!(dcc.year_fraction_dates(date, date), 0.0);
        }
    }
    ...
    fn test_thirty_360_bond() {
        ...
        let yf = DayCountConvention::Thirty360Bond.year_fraction_dates(start, end);
        assert!((yf - 60.0 / 360.0).abs() < 1e-10);
    }

    #[test]
    fn test_default() {
        let dcc = DayCountConvention::default();
        assert_eq!(dcc, DayCountConvention::Actual365Fixed);
    }
    ...
    fn test_hash() {
        ...
        set.insert(DayCountConvention::Actual365Fixed);
        set.insert(DayCountConvention::Actual360);
        set.insert(DayCountConvention::Actual365Fixed); // Duplicate
        assert_eq!(set.len(), 2);
    }
}
