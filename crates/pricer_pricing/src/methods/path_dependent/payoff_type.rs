...
//!
//! The `#[enum_dispatch]` attribute is used to auto-generate the `PathDependentPayoff`
//! trait implementation, eliminating boilerplate match statements.
...
/// ```
///
/// Uses `enum_dispatch` to automatically implement `PathDependentPayoff<T>` trait
...
impl<T: Float + Send + Sync> PathPayoffType<T> {
    ...
    // ========================================================================
    //
    // NOTE: `compute()`, `required_observations()`, and `smoothing_epsilon()` are now
    // auto-generated by the `#[enum_dispatch(PathDependentPayoff<T>)]` attribute.
    // These methods forward calls to the inner variant types automatically.
