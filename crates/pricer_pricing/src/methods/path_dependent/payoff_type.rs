...
//!
//! The `#[enum_dispatch]` attribute is used to auto-generate the
//! `PathDependentPayoff` trait implementation, eliminating boilerplate match
//! statements.
...
///
/// ```ignore
/// use pricer_pricing::path_dependent::{PathPayoffType, PathObserver, AsianParams};
///
...
/// ```
///
/// Uses `enum_dispatch` to automatically implement `PathDependentPayoff<T>`
/// trait by forwarding method calls to the inner variant types.
...
impl<T: Float + Send + Sync> PathPayoffType<T> {
    ...
    // ========================================================================
    // Type Classification Methods (inherent, not part of trait)
    // ========================================================================
    //
    // NOTE: `compute()`, `required_observations()`, and `smoothing_epsilon()` are
    // now auto-generated by the `#[enum_dispatch(PathDependentPayoff<T>)]`
    // attribute. These methods forward calls to the inner variant types
    // automatically.
