        ...
        I: CalibrationInstrument<T> + Clone,
    {
        ...
        for iter in 0..self.config.max_iterations {
            ...
            self.strategy.validate_structure(&jacobian).map_err(|e| {
                CalibrationError::NumericalInstability { message: format!("Invalid Jacobian structure: {e}") }
            })?;
            ...
            let delta = self.strategy.solve(&residuals).map_err(|e| {
                CalibrationError::NumericalInstability { message: format!("Linear solve failed: {e}") }
            })?;
        ...
        // Build final curve
        let curve = problem.build_curve(&x).map_err(|e| {
            CalibrationError::NumericalInstability { message: format!("Failed to build final curve: {e}") }
        })?;
