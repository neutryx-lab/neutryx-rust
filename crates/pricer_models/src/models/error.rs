...
impl From<ModelError> for PricingError {
    fn from(err: ModelError) -> Self {
        match err {
            ...
            #[cfg(feature = "equity")]
            ModelError::SABR(e) => PricingError::ModelFailure(e.to_string()),
            #[cfg(feature = "exotic")]
...
mod tests {
    ...
    #[cfg(feature = "equity")]
    #[test]
    fn test_sabr_error_to_model_error() {
        let sabr_err = SABRError::InvalidAlpha(-0.5);
        let model_err: ModelError = sabr_err.into();
        assert!(matches!(model_err, ModelError::SABR(_)));
    }
