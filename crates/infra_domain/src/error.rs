...
mod tests {
    ...
    #[test]
    fn test_date_and_currency_errors() {
        // DateError
        let d = DateError::InvalidDate { year: 2024, month: 2, day: 30 };
        assert_eq!(format!("{d}"), "Invalid date: 2024-02-30");
        assert_eq!(d, d.clone());
        let _: &dyn std::error::Error = &d;
        assert_eq!(format!("{}", DateError::ParseError("invalid".into())), "Date parse error: invalid");

        // CurrencyError
        assert_eq!(format!("{}", CurrencyError::UnknownCurrency("XYZ".into())), "Unknown currency: XYZ");
        assert_eq!(format!("{}", CurrencyError::ParseError("bad".into())), "Currency parse error: bad");
        assert_eq!(format!("{}", CurrencyError::SameCurrency("USD".into())), "Base and quote currencies are the same: USD");
        assert_eq!(format!("{}", CurrencyError::InvalidSpotRate), "Invalid spot rate: must be positive");
        let c = CurrencyError::UnknownCurrency("XYZ".into());
        assert_eq!(c, c.clone());
        let _: &dyn std::error::Error = &c;

        // MasterDataError from DateError
        let m: MasterDataError = DateError::InvalidDate { year: 2024, month: 2, day: 30 }.into();
        assert!(matches!(m, MasterDataError::InvalidDate(_)));
        assert!(format!("{m}").contains("2024-02-30"));
        let m2 = MasterDataError::CalendarNotFound("TARGET".into());
        assert_eq!(m2, m2.clone());
    }

    #[test]
    fn test_book_and_portfolio_errors() {
        // BookError
        assert_eq!(format!("{}", BookError::DuplicateId("B1".into())), "Duplicate BookId: B1");
        assert_eq!(format!("{}", BookError::InvalidOwnership("desk".into())), "Invalid ownership: desk");
        assert_eq!(format!("{}", BookError::InvalidType("bad".into())), "Invalid book type: bad");
        assert_eq!(format!("{}", BookError::MissingRequiredField("name".into())), "Missing required field: name");
        let b = BookError::DuplicateId("B1".into());
        assert_eq!(b, b.clone());
        let _: &dyn std::error::Error = &b;

        // PortfolioError
        assert_eq!(format!("{}", PortfolioError::DuplicateId("P1".into())), "Duplicate PortfolioId: P1");
        assert_eq!(format!("{}", PortfolioError::CircularReference("P1".into(), "P2".into())), "Circular portfolio reference detected: P1 -> P2");
        assert_eq!(format!("{}", PortfolioError::InvalidBookReference("BX".into())), "Invalid book reference: BX");
        assert_eq!(format!("{}", PortfolioError::InvalidScope("bad".into())), "Invalid portfolio scope: bad");
        let p = PortfolioError::CircularReference("P1".into(), "P2".into());
        assert_eq!(p, p.clone());
        let _: &dyn std::error::Error = &PortfolioError::DuplicateId("P1".into());
    }

    #[test]
    fn test_netting_and_exposure_errors() {
        // NettingError
        let n = NettingError::CounterpartyMismatch { expected: "CP1".into(), actual: "CP2".into() };
        assert!(format!("{n}").contains("expected CP1"));
        assert_eq!(format!("{}", NettingError::NotEnforceable("Russia".into())), "Netting not enforceable in jurisdiction: Russia");
        assert_eq!(format!("{}", NettingError::InvalidAgreement("sig".into())), "Invalid netting agreement: sig");
        assert_eq!(format!("{}", NettingError::CrossBookViolation("B1B2".into())), "Cross-book netting violation: B1B2");
        assert_eq!(n, n.clone());
        let _: &dyn std::error::Error = &NettingError::NotEnforceable("X".into());

        // ExposureError
        assert_eq!(format!("{}", ExposureError::MissingDate("2025-06-30".into())), "Missing exposure data for date: 2025-06-30");
        assert_eq!(format!("{}", ExposureError::CurrencyMismatch { expected: "USD".into(), actual: "EUR".into() }), "Currency mismatch: expected USD, got EUR");
        assert_eq!(format!("{}", ExposureError::InvalidTimeGrid("gaps".into())), "Invalid time grid: gaps");
        let e = ExposureError::MissingDate("d".into());
        assert_eq!(e, e.clone());
    ...
    fn test_validation_error() {
        // From conversions
        for (v, s) in [
            (ValidationError::from(BookError::DuplicateId("B1".into())), "B1"),
            (ValidationError::from(PortfolioError::DuplicateId("P1".into())), "P1"),
            (ValidationError::from(NettingError::NotEnforceable("Russia".into())), "Russia"),
            (ValidationError::from(ExposureError::MissingDate("2025".into())), "2025"),
        ] {
            assert!(format!("{v}").contains(s));
        }

        // Multiple
        let multi = ValidationError::Multiple(vec![
            ValidationError::Book(BookError::DuplicateId("B1".into())),
            ValidationError::Portfolio(PortfolioError::DuplicateId("P1".into())),
        ]);
        assert!(format!("{multi}").contains("Multiple"));

        // Clone + trait
        let e: ValidationError = BookError::DuplicateId("B1".into()).into();
        assert!(format!("{}", e.clone()).contains("B1"));
        let _: &dyn std::error::Error = &e;

        // Result alias
        fn f() -> ValidationResult<String> { Ok("ok".into()) }
        assert!(f().is_ok());
    }
}
