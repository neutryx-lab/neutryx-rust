...
| ファイル | 内容 |
|----------|------|
| [types.rs](crates/pricer_pricing/src/graph/types.rs) | `GraphNode`, `GraphEdge`, `ComputationGraph`, `GraphMetadata`, `GraphNodeUpdate`, `NodeType`, `NodeGroup` |
| [extractor.rs](crates/pricer_pricing/src/graph/extractor.rs) | `GraphExtractable` trait, `SimpleGraphExtractor`, `GraphBuilder` |
| [error.rs](crates/pricer_pricing/src/graph/error.rs) | `GraphError` (TradeNotFound, ExtractionFailed, Timeout) |
...
| ファイル | 内容 |
|----------|------|
| [mod.rs](crates/pricer_risk/src/portfolio/mod.rs) | `Portfolio` struct, O(1)ルックアップ, Rayon並列イテレーション |
| [builder.rs](crates/pricer_risk/src/portfolio/builder.rs) | `PortfolioBuilder` (バリデーション付きビルダーパターン) |
...
| ファイル | 内容 |
|----------|------|
| [mod.rs](demo/gui/src/web/mod.rs) | `AppState`（graph_cache, graph_subscriptions含む）, Router構築 |
| [handlers.rs](demo/gui/src/web/handlers.rs) | `get_graph()` handler, `GraphCache`, `sample_trades()` |
| [websocket.rs](demo/gui/src/web/websocket.rs) | `broadcast_graph_update()`, `RealTimeUpdate::graph_update()` |
...
1. ノード重複排除アルゴリズム（ハッシュベース同一性判定）
2. LOD簡略化戦略（クラスタリング or 階層集約）
3. GraphNodeの`trade_ids`フィールド追加時のserde互換性
...
**概要**: 既存の`SimpleGraphExtractor`をPortfolio対応に拡張

**変更対象ファイル:**
- `crates/pricer_pricing/src/graph/types.rs` - GraphNode拡張
- `crates/pricer_pricing/src/graph/extractor.rs` - Portfolio対応メソッド追加
- `demo/gui/src/web/handlers.rs` - API拡張

**Trade-offs:**
- ✅ 既存コードの再利用最大化
- ✅ 学習コスト低い
- ❌ `SimpleGraphExtractor`が肥大化するリスク
- ❌ 単一責任原則に違反する可能性

### Option B: Create New PortfolioGraphExtractor

**概要**: Portfolio専用の新クラスを作成、既存Extractorをコンポジション

**新規ファイル:**
- `crates/pricer_pricing/src/graph/portfolio_extractor.rs`
- `crates/pricer_pricing/src/graph/portfolio_types.rs`
- `crates/pricer_risk/src/portfolio/sample_builder.rs`
...
- ✅ テスト容易性向上
- ❌ 新規ファイル数増加
- ❌ 既存Extractorとの重複コード可能性

### Option C: Hybrid Approach（推奨）

**概要**: 段階的実装。Phase 1でExtend、Phase 2でRefactor to New

**Phase 1:**
1. GraphNode拡張（`trade_ids`オプショナルフィールド）
2. SimpleGraphExtractorにPortfolio対応メソッド追加
3. Web API拡張

**Phase 2:**
1. PortfolioGraphExtractor分離（Phase 1の学習反映）
2. LODモード実装
3. パフォーマンス最適化
...
1. [ ] D3.js force-directed graph での大規模グラフ描画パフォーマンス調査
2. [ ] WebGL/Canvas切り替え閾値の業界標準確認
3. [ ] 既存`sample_trades()`の拡張 vs 新規`SamplePortfolioBuilder`の判断
