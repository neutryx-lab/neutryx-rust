...
- [ ] 1. enzymeモジュールをpricer_riskに移動
- [ ] 1.1 enzymeディレクトリをpricer_pricingからpricer_riskにコピー
...
- [ ] 1.2 pricer_pricingからenzymeディレクトリを削除
  - enzyme/ディレクトリ全体を削除
  - verify_enzyme.rsを削除（後続タスクで移動先に再配置）
  - _Requirements: 1.2_

- [ ] 2. Cargo.toml依存関係を更新
- [ ] 2.1 (P) pricer_risk/Cargo.tomlにenzyme依存を追加
  - llvm-sys依存をoptionalで追加（version 180, features = ["prefer-dynamic"]）
  - enzyme-ad featureを定義（dep:llvm-sysを有効化）
  - _Requirements: 2.1, 2.4_

- [ ] 2.2 (P) pricer_pricing/Cargo.tomlからenzyme依存を削除
  - llvm-sys依存を削除
  - enzyme-ad feature定義を削除
  - _Requirements: 2.2_

- [ ] 3. lib.rsモジュールエクスポートを更新
- [ ] 3.1 pricer_risk/lib.rsにenzymeモジュールを追加
...
- [ ] 3.2 pricer_pricing/lib.rsからenzyme関連を削除
  - `pub mod enzyme;`を削除
  - enzymeのre-export（`pub use enzyme::*`）を削除
  - nightly feature属性（enzyme-ad関連）を削除
  - _Requirements: 1.2, 3.4_

- [ ] 4. enzyme内部のcrate参照を更新
- [ ] 4.1 greeks.rsの内部参照をpricer_pricingパスに変更
...
- [ ] 6. docstring内のパス参照を一括更新
- [ ] 6.1 (P) enzyme/*.rs内の`pricer_pricing::enzyme`参照を`pricer_risk::enzyme`に置換
