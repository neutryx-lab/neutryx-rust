...
**サブタスク**:
- 28.1: `mod.rs`にサブモジュール宣言を追加
- 28.2: re-exportを整理
- 28.3: モジュールレベルドキュメントを更新
...
**サブタスク**:
- 29.1: `pricer_models`の該当ファイルを更新
- 29.2: Black-Scholes等が新しいインポートで動作することを確認
- 29.3: 既存テストを実行
...
**サブタスク**:
- 30.1: `Cargo.toml`を更新
- 30.2: `cargo build`を実行
- 30.3: 依存関係の競合がないことを確認
...
**サブタスク**:
- 31.1: 統合テストファイルを作成
- 31.2: 分布、積分、補間の統合テストを追加
- 31.3: ドキュメントを確認・補完
- 31.4: `cargo clippy --pedantic`を実行
