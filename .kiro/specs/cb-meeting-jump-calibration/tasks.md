...
- [x] 5. InterpolationMatrixのジャンプ対応補間拡張
- [x] 5.1 ジャンプピラーを考慮した補間マトリックス生成
...
- [x] 5.2 ジャンプ調整付き補間メソッドの実装
  - `interpolate_with_jumps`メソッドでジャンプパラメータを適用
  - DF乗算方式（DF × Π(1 + jump_i)）で累積効果を計算
  - 補間重みとジャンプ調整を分離して計算
  - _Requirements: 2.2, 3.2_

- [x] 6. CalibrationProblemのジャンプ対応拡張
- [x] 6.1 拡張パラメータベクトルでの問題構築
...
- [x] 8. GlobalBootstrapResultのジャンプ情報拡張
- [x] 8.1 カリブレーション結果にジャンプ情報を追加
...
- [x] 9. APIリクエスト/レスポンスの拡張
- [x] 9.1 (P) CurveBuildRequestにCB Meetingパラメータを追加
...
- [x] 9.2 (P) CbEventInput構造体の実装
  - 日付（ISO形式）、期待ジャンプ幅（bps）、中央銀行コードを保持
  - ±100bpsのバリデーション範囲を定義
  - camelCase JSON命名でフロントエンド互換
  - _Requirements: 1.5, 4.2_

- [x] 9.3 (P) CurveBuildResponseに実現ジャンプ情報を追加
...
- [x] 10. APIハンドラのジャンプ対応
- [x] 10.1 CB Meetingパラメータのパースとバリデーション
...
- [x] 10.2 JumpPillarへの変換と範囲フィルタリング
  - CbEventInputからJumpPillarへの変換ロジック
  - 商品テナー範囲外のイベントを除外し警告を記録
  - 複数通貨のイベントを同時処理
  - _Requirements: 4.4, 4.5_

- [x] 10.3 GlobalBootstrapper呼び出しの統合
...
- [x] 11. WebUIのジャンプ入力機能
- [x] 11.1 (P) CB Meetingイベント選択時のジャンプ入力フィールド表示
  - CB Meetingイベント選択でジャンプ幅入力フィールドを表示
  - bps単位のラベルと入力ヒントを表示
  - デフォルト値0で初期化
  - _Requirements: 1.1, 1.3_

- [x] 11.2 (P) ジャンプ有効/無効トグルスイッチの実装
  - カリブレーション設定パネルにトグルを追加
  - トグル状態に応じてジャンプ入力フィールドの表示/非表示を切り替え
  - _Requirements: 5.4, 7.4_

- [x] 12. WebUIのジャンプ可視化
- [x] 12.1 フォワードカーブ上のジャンプマーカー表示
  - Chart.jsでジャンプ日付位置にマーカーを描画
  - マーカーの色とスタイルで視認性を確保
  - _Requirements: 5.1_

- [x] 12.2 ジャンプマーカーのツールチップ表示
  - マーカーホバー時にジャンプ前後のフォワードレート値を表示
  - 期待ジャンプ幅と実現ジャンプ幅を表示
  - _Requirements: 5.2_

- [x] 12.3 ジャンプマーカークリック時の詳細情報表示
  - クリックで詳細パネルを表示
...
- [x] 13. 統合テストとエンドツーエンド検証
- [x] 13.1 ジャンプ付きカリブレーションの統合テスト
  - 単一ジャンプ付きOISカリブレーションの検証
  - 複数ジャンプ付きカリブレーションの累積効果検証
  - ジャンプなし時の既存結果との同一性検証
  - _Requirements: 2.1, 7.1_

- [x] 13.2 フォールバック動作の検証テスト
