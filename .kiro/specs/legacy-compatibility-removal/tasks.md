...
- [x] 10.1 infra_master::ids モジュールを新設
  ...
  - NettingSetId, LegalEntityId, CcpId は counterparty::ids から re-export
...
- [x] 10.3 trade.rs から TradeId 型エイリアスを削除
  - `pub type TradeId = String;` を削除
  - `use crate::ids::TradeId;` に置き換え
  - Trade 構造体と関連メソッドを更新
  - _Requirements: 10.3_

- [x] 10.4 TradeMetadata を Newtype ID で更新
...
- [x] 10.5 TradeType::Bond を Newtype ID で更新
  - issuer_id: Option<String> → Option<IssuerId>
  - _Requirements: 10.5_

- [x] 10.6 trade.rs, builder.rs, expander.rs のテストを更新
  - TradeId::new() を使用するよう変更
  - TradeMetadata のテストを ID 型で更新
  - assert_eq!(trade.id, ...) → assert_eq!(trade.id.as_str(), ...) に変更
...
- [x] 9. 最終ビルド・テスト検証
- [x] 9.1 ワークスペース全体のビルド検証
  - cargo build --workspace が成功することを確認
  - ビルド成功、clippy は既存の警告あり（本仕様の変更とは無関係）
