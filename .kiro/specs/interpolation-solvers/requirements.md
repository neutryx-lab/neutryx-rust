...
4. When `interpolate(x)` is called with a `Dual64` input, the LinearInterpolator shall correctly propagate gradient information through the computation.
...
4. When `interpolate(x)` is called with `Dual64` inputs, the CubicSplineInterpolator shall correctly propagate gradients through the spline evaluation.
...
3. The MonotonicInterpolator shall use the Fritsch-Carlson method to preserve monotonicity.
4. When `interpolate(x)` is called with `Dual64` inputs, the MonotonicInterpolator shall correctly propagate gradient information.
...
4. The `smooth_interp` function shall not use conditional branches (`if`) on Float values for segment selection.
...
3. When `interpolate(x, y)` is called with `Dual64` inputs, the BilinearInterpolator shall correctly propagate partial derivatives ∂z/∂x and ∂z/∂y.
4. The BilinearInterpolator shall provide `domain_x()` and `domain_y()` methods returning the valid interpolation ranges.
...
3. The RootFinder trait shall provide configurable `tolerance` and `max_iterations` parameters.
4. The RootFinder shall support functions of type `Fn(T) -> T` for AD compatibility.
...
3. The NewtonRaphsonSolver shall provide a `find_root_ad(f, x0)` method that automatically computes derivatives using `Dual64`.
4. When using AD mode with `Dual64`, the NewtonRaphsonSolver shall extract both function value and derivative from a single function evaluation.
...
3. The BrentSolver shall combine bisection, secant, and inverse quadratic interpolation for optimal convergence.
4. The BrentSolver shall guarantee convergence for continuous functions with a valid bracket.
...
3. Each interpolator implementation shall be in a separate submodule (e.g., `linear.rs`, `cubic_spline.rs`).
4. All public types shall include rustdoc documentation with examples.
