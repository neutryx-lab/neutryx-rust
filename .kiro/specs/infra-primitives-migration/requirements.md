...
4. The `infra_domain` crate shall export `BusinessDayConvention` enum with Following, ModifiedFollowing, Preceding, Unadjusted variants
5. When `infra_domain` types are used in pricer crates, the dependency direction shall be Pricer→Infra (not Infra→Pricer)
...
4. If `pricer_core` previously had `DayCountConvention`, the migration shall remove or deprecate it with re-export from `infra_domain`
...
4. The re-exports shall be marked with `#[deprecated]` attribute guiding users to direct `infra_domain` imports
...
4. The `infra_domain` crate shall NOT depend on any `pricer_*` crate
...
#### Acceptance Criteria
1. The `infra_domain` crate shall export `Tenor` enum or struct representing financial tenors
