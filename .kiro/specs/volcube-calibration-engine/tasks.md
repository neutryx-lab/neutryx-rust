...
- [x] 1. 基盤型とカリブレーション設定の定義
- [x] 1.1 (P) カリブレーション設定用列挙型の実装
  ...
  - **Implemented**: `crates/pricer_models/src/market/volcube/config.rs`

- [x] 1.2 (P) VolCubeConfig構造体の実装
...
- [x] 1.3 (P) volcubeモジュール構造の設定
  - `pricer_models::market`配下に`volcube/`サブモジュールを作成
  - mod.rsでpublic APIをエクスポート
  - A-I-P-Sアーキテクチャに準拠したモジュール配置を確認
  - _Requirements: 8.1, 8.2_
  - **Implemented**: `crates/pricer_models/src/market/volcube/mod.rs`

- [x] 2. エラーハンドリング基盤
- [x] 2.1 CalibrationError型の拡張
  ...
  - **Implemented**: `crates/pricer_models/src/market/volcube/error.rs`

- [x] 2.2 カリブレーション診断情報の実装
  - カリブレーション結果に診断情報（iterations, residuals, parameter_values）を付与
  - エラー発生時に診断情報を含める仕組みを構築
  - _Requirements: 7.5_
  - **Implemented**: `crates/pricer_models/src/market/volcube/error.rs` (CalibrationDiagnostics)
...
- [x] 3.1 VolCubeKeyの実装
  ...
  - _Requirements: 5.1_
  - **Implemented**: `crates/pricer_models/src/market/volcube/cache.rs`

- [x] 3.2 VolCubeCacheの実装
...
- [x] 4. VolCubeコア実装
- [x] 4.1 SabrParameterSurface構造体の実装
  - expiry-tenor格子上のSABRパラメータ（alpha, rho, nu）を保持
  - Bilinear補間によるパラメータ取得メソッドを実装
  - 既存のSABR calibrationパターンとの整合性を確保
  - _Requirements: 1.2, 1.5_
  - **Implemented**: `crates/pricer_models/src/market/volcube/sabr_surface.rs`

- [x] 4.2 VolCube構造体とVolatilityCubeトレイトの実装
  ...
  - **Implemented**: `crates/pricer_models/src/market/volcube/cube.rs`

- [x] 4.3 volatilityメソッドの実装
...
- [x] 5. VolCubeBuilder実装
- [x] 5.1 Builder APIの実装
  - new()で空のBuilderを作成
  ...
  - _Requirements: 1.3_
  - **Implemented**: `crates/pricer_models/src/market/volcube/builder.rs`

- [x] 5.2 build()メソッドとカリブレーションロジックの実装
...
- [x] 6. 確率密度関数の実装
- [x] 6.1 BreedenLitzenberger計算モジュールの実装
  - Breeden-Litzenberger公式（f(K) = e^(rT) × d²C/dK²）を実装
  - Black-Scholesコール価格計算を活用
  - 中心差分による二次微分を実装
  - delta_kパラメータによる微分幅の調整を可能にする
  - _Requirements: 3.1_
  - **Implemented**: `crates/pricer_models/src/market/volcube/breeden_litzenberger.rs`

- [x] 6.2 probability_densityとcumulative_probabilityの実装
  ...
  - **Implemented**: `crates/pricer_models/src/market/volcube/breeden_litzenberger.rs`, `cube.rs`

- [x] 7. 計算グラフ統合
- [x] 7.1 GraphExtractable実装
  ...
  - **Implemented**: `crates/pricer_models/src/market/volcube/graph.rs`

- [x] 8. 統合と拡張性
- [x] 8.1 (P) VolSurfaceEnumへのCube variant追加
  - 既存のVolSurfaceEnum（static dispatch）にCubeSlice(VolCubeSlice)を追加
  - 既存の2D VolatilitySurfaceトレイトとの互換性を維持
  - pricer_coreの数学ユーティリティ（interpolators, solvers）との連携を確認
  - _Requirements: 8.3, 8.4, 8.5_
  - **Implemented**: `crates/pricer_models/src/market/surfaces/vol_surface_enum.rs`
...
- [x] 8.2 (P) 拡張性基盤の整備
  ...
  - **Implemented**: `crates/pricer_models/src/market/volcube/calibrator.rs`
...
- [x] 9.1 (P) 単体テストの実装
  ...
  - _Requirements: 9.3_
  - **Implemented**: Tests in all module files

- [x] 9.2 (P) プロパティベーステストの実装
  ...
  - **Implemented**: `crates/pricer_models/src/market/volcube/proptest_tests.rs`

- [x] 9.3 (P) ベンチマークとAAD検証の実装
  ...
  - **Implemented**: `crates/pricer_models/benches/volcube.rs`
...
All 9 major tasks and 23 sub-tasks have been completed. The volcube module provides:

### Core Components
- **VolCube<T>**: 3D volatility cube with SABR interpolation
- **VolCubeBuilder**: Fluent builder with cache integration
- **VolCubeCache**: Thread-safe LRU cache with statistics
- **SabrParameterSurface**: Bilinear interpolation for SABR parameters

### Supporting Modules
- **BreedenLitzenberger**: Risk-neutral PDF/CDF calculation
- **VolCubeGraphData**: D3.js-compatible computation graph export
- **VolCubeCalibrator trait**: Extensible calibration strategies

### Integration
- **VolSurfaceEnum::CubeSlice**: 2D surface interface for 3D cube
- **Feature flags**: `local-vol`, `stochastic-local-vol` for advanced models
