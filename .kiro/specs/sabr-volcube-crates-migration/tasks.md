...
- [ ] 3. VolCubeBuilder APIの更新
  - expiry対応のAPIに更新しカリブレーションを統合

- [ ] 3.1 add_quote()とadd_slice()を更新
...
- [ ] 4.1 スタンドアロン関数を削除
  ...
  - black_call_price()を削除
  - _Requirements: 7.1_

- [ ] 4.2 VolCubeBuilderを使用するハンドラに移行
  - pricer_models::builder::vol::VolCubeBuilderを使用するようにvolcube_handlersを更新
  - HTTPリクエストパラメータからSliceCalibrationConfigを構築
  - CalibrationErrorを適切なHTTPレスポンス（400, 422, 500）に変換
  - demo/gui Cargo.tomlにpricer_models依存を追加（必要な場合）
